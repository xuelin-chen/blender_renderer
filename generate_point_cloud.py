...
# find /workspace/dataset/ShapeNetCore.v2/02958343 -name '*.obj' -print0 | xargs -0 -n1 -P10 -I {} /workspace/nn_project/blender-2.79-linux-glibc219-x86_64/blender --background --python generate_point_cloud.py -- --output_folder ./car {}
#
...
if not os.path.exists(vox_mat_filename):
  ...
  retur
vox_mesh = util.mesh_from_voxels(vox_mat_filename)
vox_mesh = util.mesh_normalize(vox_mesh, norm_type=args.normalization_mode) # normalize
...
for i in range(args.nb_view):
  rot_x_angle = random.randint(0, 45)
